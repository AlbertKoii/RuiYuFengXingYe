...
import appointment from '@/store/features/store';
import { useDispatch } from 'react-redux';
import { appointmentUser } from '@/features/appointment/appointmentSlice
...
const AppointmentModal: React.FC<AppointmentModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (name && phone && area && date) {
      dispatch(appointmentUser({
        name,
        phone,
        area,
        appointmentDate: date.toISOString(), // 將日期轉換為ISO字符串
      }));
      onClose(); // 提交後關閉模態框
    }
  };
                ...
                <div className="flex flex-col space-y-1.5">
                  ...
                  <Select>
                    <SelectTrigger id="framework">
                      <SelectValue placeholder="保養維修區域" />
                    </SelectTrigger>

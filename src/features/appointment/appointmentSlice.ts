...
export const appointmentUser = createAsyncThunk(
    ...
    async (appointmentData: { 
        ...
        try {
            const response = await axios.post('http://localhost:3002/JS_UncleAir_Try', appointmentData); // 替换为你的 API URL
...
export const fetchUser = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
        try {
            const response = await axios.get('http://localhost:3002/JS_UncleAir_Try'); // 替换为你的 API URL
            return response.data; // 假设返回的数据包含用户信息
        } catch (error) {
            return rejectWithValue(error.response.data); // 返回错误信息
        }
